...
import eu.slomkowski.octoglow.octoglowd.dataharvesters.NetworkDataHarvester.Companion.createIpFromHexString
import eu.slomkowski.octoglow.octoglowd.dataharvesters.NetworkDataHarvester.Companion.getActiveInterfaceInfo
import eu.slomkowski.octoglow.octoglowd.dataharvesters.NetworkDataHarvester.Companion.parsePingOutput
import eu.slomkowski.octoglow.octoglowd.dataharvesters.NetworkDataHarvester.Companion.parseProcNetRouteFile
import eu.slomkowski.octoglow.octoglowd.dataharvesters.NetworkDataHarvester.Companion.pingAddressAndGetRtt
...
class NetworkDataHarvesterTest {
    companion object {
    ...
    fun testPingAddress(): Unit = runBlocking {
        ...
        val active = checkNotNull(getActiveInterfaceInfo())

        pingAddressAndGetRtt(pingBinary, active.name, "1.1.1.1", 5.seconds, 4)
            .apply {
                assertNotNull(this)
    ...
    fun testParsePingOutput() {
        fun readText(file: String): String = NetworkDataHarvesterTest::class.java.getResourceAsStream("/ping-output/$file").use {
            it.readToString()
        }

        parsePingOutput(readText("1.txt")).apply {
        ...
        assertFails("info about RTT not found in ping output") {
            parsePingOutput(readText("2.txt"))
        }
    }
    ...
    fun testCreateIpFromHexString() {
        ...
        fun checkIp(str: String, textual: String) {
            ...
            assertEquals(expected, createIpFromHexString(str))
        }
    ...
    fun testParseProcNetRouteFile() {
        fun getList(t: String): List<NetworkDataHarvester.RouteEntry> =
            NetworkDataHarvesterTest::class.java.getResourceAsStream("/proc-net-route/$t").use { inputStream ->
                parseProcNetRouteFile(
                    BufferedReader(
                        InputStreamReader(
                            inputStream,
