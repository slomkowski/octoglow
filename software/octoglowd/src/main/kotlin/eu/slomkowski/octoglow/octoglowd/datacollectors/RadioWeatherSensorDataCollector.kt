...
class RadioWeatherSensorDataCollector(
    private val config: Config,
    private val hardware: Hardware,
    eventBus: HistoricalValuesEvents,
) : DataCollector(logger, 5.seconds, eventBus) {
    ...
    private fun poolRemoteSensor(
        ...
        if (receivedReportResult.isFailure) {
            ...
            return StandardMeasurementReport(
                now, MAXIMAL_DURATION_BETWEEN_MEASUREMENTS,
                listOf(
                    StandardMeasurement(temperatureDbKey, Result.failure(exp)),
                    StandardMeasurement(humidityDbKey, Result.failure(exp)),
                    StandardMeasurement(weakBatteryDbKey, Result.failure(exp))
                )
            )
        }
        ...
        return StandardMeasurementReport(
            now, MAXIMAL_DURATION_BETWEEN_MEASUREMENTS,
            listOf(
                StandardMeasurement(temperatureDbKey, Result.success(receivedReport.temperature)),
                StandardMeasurement(humidityDbKey, Result.success(receivedReport.humidity)),
                StandardMeasurement(
    ...
    override suspend fun pollForNewData(now: Instant): Unit = coroutineScope {
        ...
        listOf(
            ...
            }).awaitAll().filterNotNull().forEach { publishPacket(it) }
    }
}