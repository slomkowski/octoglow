...
import eu.slomkowski.octoglow.octoglowd.daemon.frontdisplay.CryptocurrencyView
        ...
        val measurements = coinKeys.map { symbol ->
            async {
                ...
                try {
                    ...
                    StandardMeasurement(dbKey, Result.success(value))
                } catch (e: Exception) {
                    logger.error(e) { "Failed to update status on $symbol." }
                    StandardMeasurement(dbKey, Result.failure(e))
                }
            }
        }.awaitAll()

        publishPacket(StandardMeasurementReport(now, pollInterval, measurements))
    }
}