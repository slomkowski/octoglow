...
import eu.slomkowski.octoglow.octoglowd.dataharvesters.AirQualityDataHarvester
import eu.slomkowski.octoglow.octoglowd.dataharvesters.AirQualityDataSample
...
class AirQualityView(
    ...
    hardware: Hardware,
) : FrontDisplayView<AirQualityView.CurrentReport, Unit>(
    hardware,
    ...
    data class SingleStationData(
        val name: String,
        val level: AirQualityDataHarvester.AirQualityIndex?,
        val latest: Double?,
    ...
    private suspend fun getForStation(
        report: StandardDataSnapshot,
        station: ConfSingleAirStation
    ): SingleStationData? {
        val dbKey = AirQuality(station.id)

        val measurement = report.values.firstOrNull { it.type == dbKey } as? AirQualityDataSample ?: return null
