...
class NetworkDataCollector(
...
) : DataCollector(logger, 38.seconds, eventBus) {
    ...
    data class NetworkMeasurementReport(
    ...
    override suspend fun pollForNewData(now: Instant) = coroutineScope {
        ...
        publishPacket(
            NetworkMeasurementReport(
                ...
                iface,
                listOf(
                    StandardMeasurement(PingTimeGateway, gatewayPingTime.await()),
                    StandardMeasurement(PingTimeRemoteHost, remotePingTime.await()),
                )
            )
        )
    }
