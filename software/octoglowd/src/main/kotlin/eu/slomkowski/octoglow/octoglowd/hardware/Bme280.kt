...
data class Bme280measurements(
    ...
    companion object {
         ...
         */
        fun parse(cd: CompensationData, adcP: Int, adcT: Int, adcH: Int): Bme280measurements {
            ...
            return Bme280measurements(cTemp, humidity, pressure)
        }
    }
}
...
class Bme280(hardware: Hardware) : I2cDevice(hardware, 0x76, logger) {
    ...
    override suspend fun initDevice() = coroutineScope {
        doWrite(0xe0, 0xb6)
        ...
        val b1 = doTransaction(intArrayOf(0x88), 25)
        val b2 = doTransaction(intArrayOf(0xe1), 8)
    ...
    suspend fun readReport(): Bme280measurements = trySeveralTimes(3, logger, "read BME280 report") {
        ...
        Bme280measurements.parse(checkNotNull(compensationData) { "sensor is not initialized" }, adcP, adcT, adcH)
    }
}