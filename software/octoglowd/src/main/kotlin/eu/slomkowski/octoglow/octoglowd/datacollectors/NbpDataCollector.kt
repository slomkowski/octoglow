...
class NbpDataCollector(
    private val config: Config,
    eventBus: HistoricalValuesEvents,
) : DataCollector(logger, 10.minutes, eventBus) {
    ...
    data class SingleNbpCurrencyMeasurement(
        val code: String,
    ...
    companion object {
        ...
        fun createReport(code: String, rates: List<DatedPrice>, today: LocalDate): SingleNbpCurrencyMeasurement {
            ...
            return SingleNbpCurrencyMeasurement(
                code,
        ...
        suspend fun getCurrencyReport(code: String, today: LocalDate): SingleNbpCurrencyMeasurement {
            val rates = when (code) {
    ...
    override suspend fun pollForNewData(now: Instant): Unit = coroutineScope {
        val newReports: List<SingleNbpCurrencyMeasurement> = currencyKeys.map { code ->
            async {
                try {
                    getCurrencyReport(
                        code,
        ...
        publishPacket(StandardMeasurementReport(now, pollInterval, newReports))
    }
}